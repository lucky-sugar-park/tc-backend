...
//			// 암호가 일치하지 않으면 AuthenticationException 형식의 예외 발생
	...
	/**
	 * AuthenticationProvider 가 어떤 종류의 Authentication 인터페이스를 지원할 지 결정 이는
	 * authenticate() 메서드의 매개 변수로 어떤 형식이 제공될지에 따라서 달라짐
	 * <p>
	 * AuthenticationFilter 수준에서 아무것도 맞춤 구성하지 않으면(지금은 이런 케이스)
	 * UsernamePasswordAuthenticationToken 클래스가 형식을 정의함
	 */
	@Override
